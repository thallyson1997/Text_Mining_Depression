---
title: "METODO 5 - Redução parcial da Classe Marjoritária (2 * n0 = n1) e aumento da classe marjoritária usando SMOTE."
...
#Função para recombinar amostras
...
rodar_pipeline <- function(TREINO, TESTE, dir_saida) {
  ...
  for (n in 1:10) {
    cat("-> Balanceando conjunto", n, "\n")
    TREINO[[n]] <- recombina(TREINO[[n]])
    cat("-> Vetorizando conjunto", n, "\n")
    tfidf <- TFIDF(TREINO[[n]], TESTE[[n]], 250)
    TREINO[[n]] <- tfidf[[1]]
    TESTE[[n]]  <- tfidf[[2]]
    cat("-> Aplicando SMOTE no conjunto", n, "\n")
    TREINO[[n]] <- aplicar_SMOTE(TREINO[[n]])
  }
...
resultado <- rodar_pipeline(
  TREINO,
  TESTE,
  "C:/Users/usuario/OneDrive/Documentos/Text_Mining_Reddit_Depression_Post/Resultado/METODO_5"
)
```